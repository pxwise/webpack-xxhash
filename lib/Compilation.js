...
Compilation.prototype.createHash = function createHash() {
	...
	var hash = new xxhash(0xB0BAFE77);
	if(outputOptions.hashSalt)
		hash.update(new Buffer(outputOptions.hashSalt));
	...
	for(i = 0; i < chunks.length; i++) {
		...
		var chunkHash = new xxhash(0xB0BAFE77);
		if(outputOptions.hashSalt)
			hash.update(new Buffer(outputOptions.hashSalt));
		...
		hash.update(new Buffer(chunk.hash));
...
Compilation.prototype.modifyHash = function modifyHash(update) {
	...
	// var hash = require("crypto").createHash(hashFunction);
	var hash = new xxhash(0xB0BAFE77);
	hash.update(new Buffer(this.fullHash));
