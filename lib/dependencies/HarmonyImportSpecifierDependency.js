...
HarmonyImportSpecifierDependency.prototype.updateHash = function(hash) {
	...
	var importedModule = this.importDependency.module;
	hash.update(new Buffer((importedModule && importedModule.id) + ""));
	hash.update(new Buffer((importedModule && this.id) + ""));
	hash.update(new Buffer((importedModule && this.importedVar) + ""));
	hash.update(new Buffer((importedModule && this.id && importedModule.isUsed(this.id)) + ""));
	hash.update(new Buffer((importedModule && (!importedModule.meta || importedModule.meta.harmonyModule)) + ""));
	hash.update(new Buffer((importedModule && (importedModule.used + JSON.stringify(importedModule.usedExports))) + ""));
};
